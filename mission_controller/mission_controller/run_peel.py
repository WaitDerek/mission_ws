"""Legacy-style one-shot film-peel sequence."""
...
class PeelRunResult:
    """Output of one legacy-style peel sequence."""

    contact_detected: bool
    force_delta: float
    message: str = "peel pipeline completed"
...
class RunPeelMixin:
    ...
    def run_peel(self, goal_handle) -> PeelRunResult:
        ...
        _, right_pose = self._wait_for_run_poses(
            goal_handle,
            need_left=True,
            need_right=True,
            require_fresh=True,
        ...
        try:
            ...
            left_pose, right_pose = self._wait_for_run_poses(
                goal_handle,
                need_left=True,
                need_right=True,
                require_fresh=True,
            ...
            outcome = PeelRunResult(contact_detected, force_delta)
            self._pipeline_feedback(
                goal_handle, ExecutePeel, "DONE", outcome.message
            )
            return outcome
        finally:
