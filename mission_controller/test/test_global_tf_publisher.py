...
def test_realbots_urdf_contains_the_requested_camera_chain():
    ...
    transform = model.transform_from_root(
        "base_footprint", "left_arm_8_Link", zero_pose
    )
    ...
    assert math.isclose(recovered.translation[0], transform.translation[0], abs_tol=1.0e-9)
    assert math.isclose(recovered.translation[1], transform.translation[1], abs_tol=1.0e-9)
    assert math.isclose(recovered.translation[2], transform.translation[2], abs_tol=1.0e-9)
