"""MQTT ingress and status egress for starting the workflow Action."""
...
def normalize_robot_id(value: object) -> str:
    ...
    if not robot_id:
        raise ValueError("workflow start JSON robot_id must contain the robot id")
    return robot_id
    ...
    def __init__(
        ...
        *,
        ...
        if not mqtt_call_succeeded(connect_result, self._mqtt_success):
            raise RuntimeError(
                f"MQTT start connect_async returned error {connect_result!r}"
            )
